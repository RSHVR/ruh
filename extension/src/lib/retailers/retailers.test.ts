...
describe('amazonAdapter', () => {
  it('matches amazon.com and amazon.ca domains', () => {
    expect(amazonAdapter.matches('https://www.amazon.com/dp/B000123456')).toBe(true);
    expect(amazonAdapter.matches('https://www.amazon.ca/dp/B000123456')).toBe(true);
    expect(amazonAdapter.matches('https://smile.amazon.com/dp/B000123456')).toBe(true);
  });

  it('rejects non-amazon domains', () => {
    expect(amazonAdapter.matches('https://www.walmart.com/ip/123')).toBe(false);
    expect(amazonAdapter.matches('https://www.notamazon.com/dp/B000123456')).toBe(false);
    expect(amazonAdapter.matches('not a url')).toBe(false);
  });

  it('identifies product pages by /dp/ or /gp/product/', () => {
    expect(amazonAdapter.isProductPage('https://www.amazon.ca/Some-Title/dp/B000123456/ref=x')).toBe(true);
    expect(amazonAdapter.isProductPage('https://www.amazon.com/gp/product/B000123456')).toBe(true);
  });

  it('rejects non-product amazon pages', () => {
    expect(amazonAdapter.isProductPage('https://www.amazon.com/')).toBe(false);
    expect(amazonAdapter.isProductPage('https://www.amazon.com/s?k=sunscreen')).toBe(false);
  });

  it('exposes a fetchReviews capability', () => {
    expect(typeof amazonAdapter.fetchReviews).toBe('function');
  });
});

describe('getAdapter registry', () => {
  it('resolves the amazon adapter for amazon urls', () => {
    expect(getAdapter('https://www.amazon.ca/dp/B000123456')?.name).toBe('amazon');
  });

  it('returns null for unsupported domains', () => {
    expect(getAdapter('https://www.some-unconfigured-shop.example/p/1')).toBeNull();
  });
