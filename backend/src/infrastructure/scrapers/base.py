...
class BaseScraper(ABC):
    ...
    def process_client_html(
        ...
        # Reviews embedded in the product page itself (~8-10 on Amazon), merged
        # with any separately-fetched review pages the client provided.
        ...
        extracted_reviews = self._merge_reviews(product_page_reviews, fetched_reviews)
    ...
    def _merge_reviews(self, reviews1: str, reviews2: str) -> str:
        """Merge two review text blocks (product-page + fetched), keeping both.

        Overlap between the sources is tolerated; the fetched block is appended
        under a clear marker so Claude can weigh both.
        """
        if not reviews1:
            return reviews2
        if not reviews2:
